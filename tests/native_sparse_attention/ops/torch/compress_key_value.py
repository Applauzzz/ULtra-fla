...
def avgpool_compress_torch(
    ...
    y = torch.nn.functional.avg_pool1d(
        x, kernel_size=kernel_size, stride=kernel_stride, ceil_mode=True
    )
    y = rearrange(y, "b (h d) n -> b n h d", h=num_heads)
    # only keep useful part
    y = torch.cat([y[i, : y_seqlens[i]] for i in range(batch_size)], dim=0)
...
def maxminavgpool_compress(
    ...
    x = rearrange(x, "b n d -> b d n")
    # maxpool
    y_max = torch.nn.functional.max_pool1d(
        x, kernel_size=kernel_size, stride=kernel_stride, ceil_mode=True
    )
    y_max = rearrange(y_max, "b (h d) n -> b n h d", h=num_heads)
    # minpool
    y_min = -torch.nn.functional.max_pool1d(
        -x, kernel_size=kernel_size, stride=kernel_stride, ceil_mode=True
    )
    y_min = rearrange(y_min, "b (h d) n -> b n h d", h=num_heads)
    # avgpool
    y_avg = torch.nn.functional.avg_pool1d(
        x, kernel_size=kernel_size, stride=kernel_stride, ceil_mode=True
    )
...
def adaptivepool_compress(
    ...
    if pe is not None:
        x = torch.cat(
            [x, pe[None, :, None, :].expand(x.shape[0], -1, x.shape[2], -1, -1)], dim=-1
        )
