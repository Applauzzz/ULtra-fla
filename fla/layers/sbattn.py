...
from fla.ops.sb import scatter_boss_function
from fla.ops.sb import MultiHeadRouter
...
class SBAttention(nn.Module):
    ...
    def __init__(
        ...
        self.q_router = MultiHeadRouter(self.num_heads, self.num_states, self.head_dim)
        self.k_router = MultiHeadRouter(self.num_heads, self.num_states, self.head_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            ...
            sg_indices_q, top_states_q, balance_loss_q = self.q_router(q)
            sg_indices_k, top_states_k, balance_loss_k = self.k_router(k)
...
if __name__ == "__main__":
    ...
    inputs = torch.randn((B, L, D), device="cuda").to(torch.bfloat16)
