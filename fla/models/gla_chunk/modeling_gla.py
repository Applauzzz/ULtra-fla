...
from fla.layers.gla_chunk import GatedLinearAttention
        ...
        if (self.memory_efficient_checkpointing 
            ...
            for layer in self.layers:
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    # print(layer_state[-1].shape, "layer_state[-1] shape")
                if output_attentions:
...
def set_seed(seed: int = 42):
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.manual_seed_all(seed)  # 对所有GPU设备
...
def acc_test(seed: int = 42):
    ...
    model1.load_state_dict(model_state)  # 🔧 关键：加载相同的初始状态
    model1.train()
    ...
    # 如果模型有dropout等随机操作，在forward前设置种子
    torch.manual_seed(seed + 1)  # 使用不同的种子避免与输入生成冲突
    ...
    model2.train()
    # model2.disable_memory_efficient_checkpointing()  # 确保初始状态一致
    model2.enable_memory_efficient_checkpointing(chunk_size=2048)  # 🔧 启用checkpointing
    
    # 使用相同的输入
    input_ids2 = base_input_ids.clone()
    

    torch.manual_seed(seed + 1)  # 与测试1使用相同的种子
