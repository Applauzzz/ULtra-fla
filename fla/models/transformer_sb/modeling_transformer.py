...
from fla.layers.sbattn import SBAttention, MultiHeadRouter
...
class TransformerBlock(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[Any]
        ...
        hidden_states, attentions, past_key_values, balance_loss = self.attn(
...
class TransformerPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(
        ...
        if isinstance(module, (nn.Linear, nn.Conv1d)):
            ...
            nn.init.normal_(module.weight, mean=0.0, std=self.config.initializer_range)
        elif isinstance(module, MultiHeadRouter):
            nn.init.normal_(module.weight, mean=0.0, std=self.config.initializer_range)
            nn.init.zeros_(module.bias)
    ...
    def forward(
        ...
        **kwargs: Unpack[Any]
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            if use_cache:
                next_cache = layer_outputs[3 if output_attentions else 2]

            if output_attentions:
                all_attns += (layer_outputs[2],)
