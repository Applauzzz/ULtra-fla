...
from fla.models.U_Net_m.ucache import Cache, FLAGenerationMixin
...
class TransformerModel(TransformerPreTrainedModel):
    ...
    def __init__(
        ...
        self.sample_rate = 4
        assert isinstance(config.window_size, list)
        self.sample_pos = [index for index, value in enumerate(config.window_size) if value is None]
        ...
        num_conn = len(self.sample_pos)
        self.connects = nn.ModuleList([nn.Linear(in_features=config.hidden_size, out_features=self.sample_rate*config.hidden_size) for _ in range(num_conn)])
    ...
    def forward(
        ...
        **kwargs: Unpack[Any]
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx in self.sample_pos:
                sample = True
                conn_idx = self.sample_pos.index(idx)
            else:
                sample = False
            ...
            if (mode == "prefill") or (token_update == True) or not sample:
                ...
                if not sample:
                    ...
                    #         layer_idx=idx,
                    #         )[0]
                    out = self.connects[conn_idx](layer_outputs[0]).reshape(batch_size, layer_outputs[0].shape[1] * self.sample_rate, -1)[:, :seqlens, :]
                    hidden_states = (out + residual).contiguous()
