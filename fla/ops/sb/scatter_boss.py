...
class BOSS_Attention(torch.autograd.Function):
    ...
    def forward(
        ...
        router_weight: torch.Tensor, # [B L H], float32
        num_classes: int,
        local_size: int,
        global_size: int,
        ...
        if boss_info.valid:
            ...
            # o2_boss, lse2_boss = scatter_boss_fwd(
            #     q=q,
            #     k=k,
            #     v=v,
            #     softmax_scale=softmax_scale,
            ...
            CHUNK_SIZE = 128
            if USE_CUSEQLENS:
                weighted_merge[(boss_info.B, H, triton.cdiv(boss_info.max_seqlen, CHUNK_SIZE))](
                    o=o,
                    lse=lse,
                    o_boss=o_boss,
                    lse_boss=lse_boss,
                ...
                weighted_merge[(boss_info.B, H, triton.cdiv(boss_info.L_S, CHUNK_SIZE))](
                    o=o,
                    lse=lse,
                    o_boss=o_boss,
                    lse_boss=lse_boss,
    ...
    def backward(
        ...
        if boss_info.valid:
            ...
            drouter_weight = torch.zeros_like(router_weight)
            do_boss = torch.zeros_like(do)
            CHUNK_SIZE = 128
            if USE_CUSEQLENS:
                dweight_scatter[(boss_info.B, H, triton.cdiv(boss_info.max_seqlen, CHUNK_SIZE))](
                    do=do,
                    o=o,
                    o_boss=o_boss,
                    do_boss=do_boss,
                ...
                dweight_scatter[(boss_info.B, H, triton.cdiv(boss_info.L_S, CHUNK_SIZE))](
                    do=do,
                    o=o,
                    o_boss=o_boss,
                    do_boss=do_boss,
            ...
            dk += dk_boss
            dv += dv_boss
        return dq, dk, dv, None, None, drouter_weight, None, None, None, None
...
def scatter_boss_bwd(
    ...
    scatter_boss_bwd_dkv_kernel[grid](
        q=q,
        k=k,
        v=v,
        do=do,
    ...
    NT = triton.cdiv(max_bin_q, CHUNK_SIZE)
    grid = (num_samples*H, NUM_STATES, NT)
    scatter_boss_bwd_dq_kernel[grid](
        q=q,
        k=k,
        v=v,
        do=do,
...
@triton.jit
def scatter_boss_bwd_dq_kernel(
    q, # [B L H D] / [1 L H D]
    k, # [B L H D] / [1 L H D]
    v, # [B L H D] / [1 L H D]
    do, # [B L H D] / [1 L H D]
...
@triton.jit
def scatter_boss_bwd_dkv_kernel(
    q, # [B L H D] / [1 L H D]
    k, # [B L H D] / [1 L H D]
    v, # [B L H D] / [1 L H D]
    do, # [B L H D] / [1 L H D]
...
@triton.jit
def scatter_boss_bwd_dqkv_kernel(
    q, # [B L H D] / [1 L H D]
    k, # [B L H D] / [1 L H D]
    v, # [B L H D] / [1 L H D]
    do, # [B L H D] / [1 L H D]
    ...
    for m_position in range(M_START, M_END, BLOCK_M):
        ...
        b_qk = tl.dot(b_q, tl.trans(b_k))
        rel_map = b_lmd[:, None] - kidx_offset[None, :]
        b_qk = tl.where((rel_map >= 0) & (rel_map < GLOBAL_SIZE), b_qk * softmax_scale, float("-inf"))
        # NOTE numerical stablization with global logsumexp
        b_p = tl.exp(b_qk - b_lse[:, None])
        acc_dv += tl.dot(tl.trans(b_p.to(b_do.dtype)), b_do).to(acc_dv.dtype)
        b_dp = tl.dot(b_do.to(b_v.dtype), tl.trans(b_v))
        b_dqk = b_p * (b_dp - b_delta[:, None]) * softmax_scale
        # NOTE accumulate dq
        b_dq = tl.dot(b_dqk.to(b_k.dtype), b_k).to(b_k.dtype)
        tl.atomic_add(p_dq, b_dq, mask=qidx_mask[:, None], sem="relaxed")
        # NOTE accumulate dk
        acc_dk += tl.dot(tl.trans(b_dqk.to(b_q.dtype)), b_q).to(acc_dk.dtype)
        # NOTE update the index a BLOCK_M offset
        qidx_offset += BLOCK_M
...
def weighted_merge(
    ...
    b_o = b_o1 + ((b_o2 * b_w[:, None]) - b_o1) * b_dl[:, None]   
    tl.debug_barrier()
...
def dweight_scatter(
    ...
    tl.debug_barrier()
    tl.store(p_dob, b_dob.to(p_dob.dtype.element_ty), boundary_check=(0, 1))
    ...
    for b in range(B):
        ...
        for h in range(H):
            o1, o2, lse = head_attention(
                ...
                router_weight=router_weight[b, :, h],
                local_size=local_size,
                global_size=global_size,
                cu_trunclens=cu_trunclens,
                cu_seqlens=cu_seqlens,
...
if __name__ == "__main__":
    ...
    # # router_weight = torch.ones((B, L, H), dtype=torch.float32, device="cuda", requires_grad=True)
    ...
    o_ref, o_boss_ref, lse_ref = scatter_boss_attention_torch_ref(
        ...
        top_states_q=top_states_q,
        top_states_k=top_states_k,
        router_weight=router_weight_ref,
        num_classes=num_states,
        local_size=local_size,
        global_size=global_size,
        cu_seqlens=cu_seqlens,
    ...
    o, o_boss, lse = scatter_boss_function(
        q=q,
        k=k,
        v=v,
        top_states_q=top_states_q,
    ...
    # print("==============================")
    # print("scatter_boss_attention")
    # print("==============================")
    # for i in range(10):
    #     torch.cuda.synchronize()
    #     start_time = time.time()
    #     o, _, _ = scatter_boss_function(
    #         q=q,
    #         k=k,
    #         v=v,
    #         top_states_q=top_states_q,
    ...
    #     loss.backward()
    #     torch.cuda.synchronize()
    #     elapse = (time.time() - start_time) * 1000
    #     print(elapse)
    #     q.grad = None
    #     k.grad = None
    #     v.grad = None
    ...
    # print("==============================")
    # print("sliding_window_attention")
    # print("==============================")
    # for i in range(10):
    #     torch.cuda.synchronize()
    #     start_time = time.time()
